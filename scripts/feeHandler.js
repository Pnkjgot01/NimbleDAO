...
function kncWeiToNMBString(NMBWei) {
...
async function enoughReserveFeesToBurn(reserveAddress) {
    ...
    console.log("reserveFeeToBurn", kncWeiToNMBString(reserveFeeToBurn))
...
async function enoughWalletFeesToSend(reserveAddress, walletAddress)
{
    ...
    console.log("walletFeeToSend", kncWeiToNMBString(walletFeeToSend))
...
async function validateReserveNMBWallet(reserveAddress) {
    ...
    let reserveWalletBalance = await kncTokenContract.methods.balanceOf(reserveNMBWallet).call();
    console.log("reserveWalletBalance", kncWeiToNMBString(reserveWalletBalance));

    let reserveWalletAllowance = await kncTokenContract.methods.allowance(reserveNMBWallet, feeBurnerAddress).call();
    console.log("reserveWalletAllowance", kncWeiToNMBString(reserveWalletAllowance));

    let walletUsableNmb = BigNumber.min(reserveWalletAllowance, reserveWalletBalance)
    console.log("walletUsableNmb", kncWeiToNMBString(walletUsableNmb));
    ...
    console.log("totalFeesToBurnAndSend", kncWeiToNMBString(totalFeesToBurnAndSend))

    if (BigNumber(walletUsableNmb).lt(totalFeesToBurnAndSend))
    {
        console.log()
        console.log()
        console.log("validation error. walletUsableNmb " + kncWeiToNMBString(walletUsableNmb) + " is less than totalFeesToBurnAndSend " + kncWeiToNMBString(totalFeesToBurnAndSend))
        console.log()
        console.log()
        errors += 1
        return false
...
function getConfig() {
    try{
        ...
        kncTokenAddress = jsonOutput["tokens"]["NMB"]["address"]
