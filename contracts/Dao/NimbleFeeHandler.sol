...
contract NimbleFeeHandler is INimbleFeeHandler, Utils5, DaoOperator, ReentrancyGuard {
    ...
    /// @dev use to get rate of NMB/ETH to check if rate to burn knc is normal
    ...
    function burnNmb() external onlyNonContract returns (uint256 kncBurnAmount) {
        ...
        emit NmbBurned(kncBurnAmount, ETH_TOKEN_ADDRESS, srcAmount);
    ...
    function validateEthToNmbRateToBurn(uint256 rateEthToNmb) internal view {
        ...
        uint256 sanityEthToNmbRate = PRECISION.mul(PRECISION).div(kncToEthRate);
