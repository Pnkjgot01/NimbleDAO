...
async function readFeeBurnerDataForReserve(feeBurnerAddress, reserveAddress, index, isNimbleReserve) {
    ...
    let kncWalletBalance = await kncInst.methods.balanceOf(NMBWallet).call();
    let walletTokenBalance = await getAmountTokens(kncWalletBalance.valueOf(), jsonNMBAddress);
    ...
    if (isNimbleReserve) {
        let NMBAddress = (await FeeBurner.methods.knc().call()).toLowerCase();
        ...
        myLog((kncPerEthRate.valueOf() == 0), (kncPerEthRate != jsonNMB2EthRate), ("kncPerEthRate: " + kncPerEthRate));
    ...
    myLog(0, 0, "Fees payed so far by reserve (burn + tax): " + await getAmountTokens(payedSoFar, jsonNMBAddress) + " knc tokens.");
