...
contract('NimbleDAO fuzz', function (accounts) {
  before('one time init: Stakers, NimbleStaking, NMB token', async () => {
    ...
    stakers = accounts.slice(1); // first account used to mint NMB tokens
    ...
    // 10k NMB token
    let nmbTweiDepositAmount = new BN(10000).mul(precisionUnits);
    ...
    for (let i = 0; i < stakers.length; i++) {
      ...
      await nmbToken.approve(stakingContract.address, maxAllowance, {from: stakers[i]});
      expectedResult = await nmbToken.allowance(stakers[i], stakingContract.address);
      Helper.assertEqual(expectedResult, maxAllowance, 'staker did not give sufficient allowance');
      totalNMB = totalNMB.add(nmbTweiDepositAmount);
    }
    // burn the rest of nmb, so the random campaign can be success
    await nmbToken.burn(await nmbToken.balanceOf(accounts[0]));

    Helper.assertEqual(totalNMB, await nmbToken.totalSupply(), 'total token in account is not match with total suppy');
  ...
  async function deposit (currentBlockTime, epoch) {
    result = await StakeGenerator.genDeposit(nmbToken, stakers);
