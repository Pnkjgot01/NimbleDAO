...
contract('NimbleDao', function(accounts) {
  before("one time init", async() => {
    ...
    await nmbToken.transfer(victor, mulPrecision(1000000));
    await nmbToken.transfer(mike, mulPrecision(1000000));
    await nmbToken.transfer(loi, mulPrecision(1000000));
    await nmbToken.transfer(poolMaster, mulPrecision(1000000));
    await nmbToken.transfer(poolMaster2, mulPrecision(1000000));
  });
  ...
  const setupSimpleStakingData = async() => {
    // approve tokens
    await nmbToken.approve(stakingContract.address, mulPrecision(1000000), {from: victor});
    await nmbToken.approve(stakingContract.address, mulPrecision(1000000), {from: mike});
    await nmbToken.approve(stakingContract.address, mulPrecision(1000000), {from: loi});
    await nmbToken.approve(stakingContract.address, mulPrecision(1000000), {from: poolMaster});
    await nmbToken.approve(stakingContract.address, mulPrecision(1000000), {from: poolMaster2});
  ...
  const setupTokenWithSupply = async(supply) => {
    nmbToken = await TestToken.new("test token", 'tst', 18, {from: accounts[0]});

    let totalSupply = await nmbToken.totalSupply();
    let burnAmount = totalSupply.sub(new BN(supply));

    await nmbToken.burn(burnAmount, {from: accounts[0]});

    Helper.assertEqual(supply, await nmbToken.totalSupply(), "total supply is invalid");

    await nmbToken.transfer(victor, supply.div(new BN(5)));
    await nmbToken.transfer(mike, supply.div(new BN(5)));
    await nmbToken.transfer(loi, supply.div(new BN(5)));
    await nmbToken.transfer(poolMaster, supply.div(new BN(5)));
    await nmbToken.transfer(poolMaster2, supply.div(new BN(5)));
  }

  const resetSetupForNMBToken = async() => {
    nmbToken = await TestToken.new("test token", 'tst', 18, {from: accounts[0]});
    await nmbToken.transfer(victor, mulPrecision(1000000));
    await nmbToken.transfer(mike, mulPrecision(1000000));
    await nmbToken.transfer(loi, mulPrecision(1000000));
  ...
  const simpleSetupToTestThreshold = async(mikeStake, victorStake, loiStake, percentage) => {
    ...
    let burnAmount = (await nmbToken.totalSupply()).sub(new BN(totalSupply));
    await nmbToken.burn(burnAmount, {from: accounts[0]});

    await nmbToken.transfer(mike, initMikeStake);
    await nmbToken.transfer(victor, initVictorStake);
    await nmbToken.transfer(loi, initLoiStake);
  ...
  describe("#Handle Withdrawal tests", () => {
    ...
    it("Test handle withdrawal updates correct data after withdraw - with delegation", async function() {
      ...
      await nimbleDao.vote(1, 2, {from: victor});
      // note: Loi already withdraw 10 nmb
    ...
    it("Test handle withdrawal should revert when sender is not staking", async function() {
      nimbleDao = await NimbleDaoContract.new(
        ...
        nmbToken.address, minCampPeriod,
  ...
  describe("#Submit Campaign tests", () => {
    it("Test submit campaign returns correct data after created", async function() {
      ...
      let totalSupply = await nmbToken.INITIAL_SUPPLY();
      ...
      for(let id = 0; id <= 2; id++) {
        ...
        Helper.assertEqual(totalSupply, data.totalNMBSupply, "total supply is incorrect");
        ...
        // burn NMB to reduce total supply value
        await nmbToken.burn(mulPrecision(1000000));
    ...
    it("Test submit new campaign for next epoch, data changes as expected", async function() {
      ...
      Helper.assertEqual(await nmbToken.totalSupply(), data.totalNMBSupply, "total supply is incorrect");
  ...
  describe("#Cancel Campaign tests", () => {
    ...
    it("Test cancel campaign should update correct data after cancelled", async function() {
      ...
      for(let id = 0; id < 2; id++) {
        ...
        Helper.assertEqual(campData.totalNMBSupply, 0, "camp details should be deleted");
    ...
    it("Test cancel campaign correctly for network fee camp", async function() {
      ...
      Helper.assertEqual(campData.totalNMBSupply, await nmbToken.totalSupply(), "camp details should be correct");
      ...
      await nmbToken.burn(mulPrecision(100));
      await submitNewCampaign(nimbleDao,
  ...
  describe("#Conclude Campaign tests", () => {
    ...
    it("Test get winning option total supply is 0", async function() {
      ...
      await nmbToken.burn(totalSupply, {from: accounts[0]});
  ...
  describe("#Constructor tests", () => {
    it("Test correct data is set after deployment", async function() {
      ...
      Helper.assertEqual(await nimbleDao.nmbToken(), nmbToken.address, "NMB token is wrong");
      ...
      Helper.assertEqual(await nimbleDao.nmbToken(), await stakingContract.nmbToken(), "nmb token is wrong");
