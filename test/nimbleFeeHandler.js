...
let ethToNmbPrecision = precisionUnits.div(new BN(200)); // 1 eth --> 200 knc
...
contract('NimbleFeeHandler', function(accounts) {
    before("Setting global variables", async() => {
        ...
        knc = await Token.new("NimbleNetworkCrystal", "NMB", NMB_DECIMALS);
        await knc.transfer(proxy.address, oneNmb.mul(new BN(10000)));
        await Helper.sendEtherWithPromise(accounts[9], proxy.address, oneEth.mul(new BN(100)));

        await proxy.setPairRate(ethAddress, knc.address, ethToNmbPrecision);
    ...
    beforeEach("Deploy new feeHandler instance", async() => {
        ...
        await sanityRate.setLatestNmbToEthRate(kncToEthPrecision);
    ...
    describe("should test events declared in feeHandler", async() => {
        ...
        it("NmbBurned", async() => {
            ...
            let expectedEthtoNmbRate = (await proxy.getExpectedRate(ethAddress, knc.address, burnPerCall)).expectedRate;

            expectEvent(txResult, "NmbBurned", {
                kncTWei: (burnPerCall.sub(burnPerCall.mul(networkFeeBps).div(BPS))).mul(expectedEthtoNmbRate).div(precisionUnits),
                token: ethAddress,
                amount: burnPerCall
            });
        });
    ...
    describe("test handle fees and claiming rebate / reward / fee", async() => {
        ...
        describe("burning", async() => {
            ...
            it("reverts if malicious NMB token is used, and burning fails", async() => {
                ...
                await proxy.setPairRate(badNMB.address, ethAddress, kncToEthPrecision);
        ...
        describe("burn config params test", async() => {
            ...
            it("test returns correct latest kncToEth rate from sanity rate", async function() {
                ...
                await newSanity.setLatestNmbToEthRate(kncToEthPrecision);
                Helper.assertEqual(kncToEthPrecision, await feeHandler.getLatestSanityRate());
    ...
    describe("Update Network Proxy", async() => {
        ...
        it("Test burn NMB after updating proxy", async() => {
            ...
            await knc.transfer(tempProxy.address, oneNmb.mul(new BN(10000)));
            await Helper.sendEtherWithPromise(accounts[9], tempProxy.address, oneEth);

            await tempProxy.setPairRate(ethAddress, knc.address, ethToNmbPrecision);
