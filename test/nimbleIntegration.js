...
contract('Proxy + Network + MatchingEngine + FeeHandler + Staking + NimbleDao integrations', function(accounts) {
    ...
    const global_init = async() => {
        ...
        // FeeHandler init
        feeHandler = await FeeHandler.new(daoSetter, networkProxy.address, network.address, NMB.address, burnBlockInterval, daoSetter);
    ...
    const setupSimpleStakingData = async() => {
        // approve tokens
        await NMB.approve(stakingContract.address, mulPrecision(1000000), {from: victor});
        await NMB.approve(stakingContract.address, mulPrecision(1000000), {from: mike});

        await NMB.transfer(mike, initMikeStake);
        await NMB.transfer(victor, initVictorStake);
